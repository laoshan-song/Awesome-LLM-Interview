...
**细化理解：** RoPE 把位置信息注入到 Q/K 的旋转相位中，使 attention 分数天然包含相对位置信息。它不像绝对位置 embedding 那样只给每个位置加一个向量，而是让不同位置之间的相对距离影响点积结果。现代 LLM 喜欢 RoPE，是因为它和 decoder-only attention 兼容性好，也更方便做一定程度的长度扩展。
...
![RoPE 原论文图：旋转位置编码实现方式](https://ar5iv.labs.arxiv.org/html/2104.09864/assets/x1.png)

> 图源：`RoFormer: Enhanced Transformer with Rotary Position Embedding` 论文 HTML 图。
...
**细化理解：** 长度外推不是把上下文窗口数字改大就行。训练时没见过的超长位置会改变 RoPE 频率分布，导致注意力模式失真；PI、NTK-aware、YaRN、LongRoPE 等方法本质上是在调整位置尺度，让模型在更长位置上仍能保持相对距离可解释。实际生产还要配合长上下文继续训练和检索式上下文管理。
...
**细化理解：** ALiBi 通过给 attention 分数加入随距离增大的线性惩罚，让模型天然偏向近邻上下文，同时保留访问远处 token 的能力。它不需要显式位置 embedding，因此外推更直接；但现代开源 LLM 更多采用 RoPE，因为 RoPE 在主流架构和训练实践中形成了更成熟的生态。
