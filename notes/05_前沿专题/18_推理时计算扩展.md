...
**细化理解：** 推理时计算扩展是在不改参数的情况下，用更多采样、搜索、验证、反思或工具调用换取更高质量答案。它适合数学、代码、规划等高价值任务，因为这些任务常能通过 verifier 或执行结果判断答案。代价是延迟和成本增加，所以需要按任务价值动态启用。
...
| 资源 | 链接 | 说明 |
|------|------|------|
| 📄 Paper | [DeepSeek-R1: Incentivizing Reasoning Capability in LLMs via RL](https://arxiv.org/abs/2501.12948) | 推理模型与 RL 训练的核心论文 |
