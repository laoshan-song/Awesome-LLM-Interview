...
![FlashAttention 原论文图：HBM / SRAM 层级与分块 attention](https://ar5iv.labs.arxiv.org/html/2205.14135/assets/x1.png)

> 图源：`FlashAttention: Fast and Memory-Efficient Exact Attention with IO-Awareness` 论文 HTML 图。核心信息是：FlashAttention 通过分块和片上 SRAM 复用，减少对 HBM 的读写。
...
**细化理解：** FlashAttention 不改变 attention 的数学定义，而是通过分块计算和减少 HBM 读写来提升效率。标准 attention 显式保存大矩阵，长序列下非常耗显存；FlashAttention 让 softmax 和矩阵乘在更高效的内存访问模式中完成，因此常用于训练和长 prompt prefill 加速。
...
**工程细节：** Continuous batching 允许服务在 decode 过程中动态加入新请求，减少 GPU 等待。在线流量中每个请求的输入长度、输出长度和到达时间都不同，静态 batch 很容易被最慢请求拖住。连续批处理要配合 KV Cache 分配、取消请求回收和公平调度，才能稳定提升吞吐。
...
| 指标 | 含义 | 常见瓶颈 |
|------|------|----------|
| TTFT | Time To First Token，首 token 延迟 | prefill、排队、长 prompt |
| TPOT | Time Per Output Token，单 token 生成延迟 | decode、KV Cache、显存带宽 |
| Throughput | 单位时间总 token 数 | batching、调度、并发 |
| GPU Utilization | GPU 利用率 | batch 太小、请求长度不均、CPU 调度 |

面试里不要只说“推理变快”，要说清楚到底是 TTFT、TPOT、吞吐还是并发能力变好。
...
1. 长 prompt 首 token 慢：优先看 chunked prefill、prefix caching、prefill/decode 分离
2. 输出 token 慢：优先看 KV Cache、batching、量化、GQA/MQA
3. 并发上不去：优先看 continuous batching、PagedAttention、请求调度和限流策略
