...
**细化理解：** 大模型训练需要同时解决显存容量、训练时间和数据吞吐。单卡装不下参数和优化器状态时，需要 ZeRO/FSDP；单卡算力不够时，需要数据并行扩大吞吐；单层矩阵太大时，需要张量并行；层数太多时，可以考虑流水线并行。并行策略本质是在计算、通信和显存之间折中。
...
**细化理解：** 数据并行让每张卡处理不同 batch，然后同步梯度。DDP 的关键成本是 AllReduce 通信，模型越大、卡越多，通信越可能成为瓶颈。为了维持训练稳定性，扩大全局 batch 后通常要配合学习率缩放、warmup 和梯度累积。
...
| 📖 DeepSpeed Docs | [ZeRO optimization](https://www.deepspeed.ai/tutorials/zero/) | 官方 ZeRO 分阶段优化教程 |
