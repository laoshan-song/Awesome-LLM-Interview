...
**细化理解：** SFT 的本质是让模型模仿高质量指令数据中的回答分布。它通常不会让模型获得大量新知识，而是让模型学会如何按用户问题组织答案、遵循角色格式、调用工具或输出结构化结果。SFT 数据质量比数量更关键，低质量样本会把模板化、啰嗦、幻觉和错误格式一起教给模型。
...
| 📖 Hugging Face TRL | [SFT Trainer](https://huggingface.co/docs/trl/sft_trainer) | 官方 SFTTrainer 用法与数据格式 |
